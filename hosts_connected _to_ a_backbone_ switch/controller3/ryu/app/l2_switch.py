...
class L2Switch(app_manager.RyuApp):
    ...
    def lldp_packet(self, datapath, hw_addr, port):
        ...
        ttl = lldp.TTL(ttl=1)
        controller_id = lldp.SystemName(system_name=b"C3.C2.C0")
        end = lldp.End()
    ...
    def host_reachability(self, host):
        try:
            ...
            length = nx.single_source_dijkstra_path_length(self.net, host)
            for edge_sw in self.edge_switches:
                host_edge_dist = length[edge_sw]
                list.append((edge_sw, host_edge_dist))
            hosts[host] = list
...
class RESTController(ControllerBase):
    ...
    def find_shortest_path(self, req, **kwargs):
        ...
        path = nx.shortest_path(controller_data.net, source=str(egressSwitch), target=str(ingressSwitch), method="dijkstra")
        print("PATH",path)
